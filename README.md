...
- ### 동시성 처리
  ...
  - 따라서 로직 처리 시 팀 별 매칭 및 요청 진행 횟수와 각각의 최대 횟수를 비교한 후 작다면 팀 별 진행 횟수를 +1 만큼 증가시키는 로직으로 처리했습니다.
  - 이 때 각각의 최대값에 근접했을때, 동시에 성사 & 요청되면 동시성 문제가 발생하는 것을 확인했습니다.
  - 따라서, 이러한 문제를 해결하기 위해 Redis에서 제공하는 분산락(Redisson) 방식을 사용하여 동시성 문제를 해결했습니다.
  - fromTeamId, toTeamId 각각 Redisson 클라이언트를 통해 락 객체를 생성하고, redisson을 사용해 두 락 객체를 묶어 다중 락을 생성해 동시성 문제를 처리했습니다.
  - 즉, 요청 처리 및 과팅 매칭 성사 로직 처리 시 2개의 테이블 Row에 락을 걸어(실제로는 DB에 거는 것이 아닌 Redis에서 처리) 동시성 문제를 해결했습니다.
