package com.everyTing.team.domain;

import com.everyTing.team.adapter.out.persistence.entity.TeamRequestEntity;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import java.util.List;
import java.util.stream.Collectors;
import lombok.Getter;

@Getter
@JsonInclude(Include.NON_NULL)
public class TeamRequests {

    private final List<TeamRequest> teamRequests;

    private TeamRequests(List<TeamRequest> teamRequests) {
        this.teamRequests = teamRequests;
    }

    public static TeamRequests from(List<TeamRequestEntity> entities) {
        return new TeamRequests(
            entities.stream()
                    .map(TeamRequest::from)
                    .collect(Collectors.toUnmodifiableList()));
    }
}
